...
const BaseCard: React.FC<NavbarItem> = ({ icon: Icon, label, subLabel, to, queryParams }) => {
  return (
    <Link href={{
        pathname: to,
        query:{...queryParams}
      ...
      <div className="w-[360px] h-[80px] cursor-pointer border border-[#d9d9d9] rounded-xl p-4 flex items-center gap-7 hover:shadow-md transition-shadow duration-150 bg-white">
        ...
        <span className="text-body2 font-medium text-gray-900">{subLabel}</span>
        </div>
      </div>
    </Link>
  );
...
/** Static Skeleton: <DashboardCard.Skeleton /> */
DashboardCard.Skeleton = ({ width = 360, height = 80 }: SkeletonProps) => (
  <div
