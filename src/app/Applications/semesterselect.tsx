...
const SemesterSelect: React.FunctionComponent<PageProps> = ({ semester, setSemester }) => {
      ...
      <FormControl sx={{ m: 1, width: 336 }}>
        <Select
          ...
          renderValue={(selected) => {
            if (selected === '') {
              return <em style={{ fontFamily: 'SF Pro Display' }}>Semester: <span style={{ fontWeight: "bold", letterSpacing: "-0.75px" }}>{semester}</span></em>;
            }
        ...
        >
          {names.map((name) => (
            ...
            >
              <em style={{ fontFamily: 'SF Pro Display', letterSpacing: "-0.75px" }}>{name}</em>
            </MenuItem>
          ))}
        </Select>
      </FormControl>
