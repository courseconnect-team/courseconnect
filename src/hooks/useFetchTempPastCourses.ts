...
const useFetchTempPastCourses = (
  ...
  useEffect(() => {
    const fetchPastCourses = async () => {
      ...
      if (!name || selectedYears.length === 0) {
        setPastCourses([]);
        return;
      }
    ...
    fetchPastCourses();
  }, [selectedYears, name]); // Use a stable dependency for selectedYears

  return { pastCourses, loadingPast, error };
};

export default useFetchTempPastCourses;
